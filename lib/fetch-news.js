...
function calcHotScore(base, minsAgo, trust) { var hoursSince = (minsAgo || 0) / 60; var timeFresh = Math.max(0, 24 - hoursSince) / 24; var authority = { A: 1.0, B: 0.8, C: 0.4 }[trust] || 0.8; return Math.round(timeFresh * 40 + authority * 30 + (base || 50) * 0.3); };
...
async function fetchNewsApi(category) {
  const sources = 'bloomberg,reuters,bbc-news,cnn,the-guardian-uk,associated-press,al-jazeera-english,the-washington-post,abc-news,time';
  const url = NEWSAPI_BASE + '/everything?sources=' + sources + '&pageSize=100&sortBy=publishedAt&apiKey=' + NEWSAPI_KEY;
  try {
...
const CCTV_ENDPOINTS = [
  ...
  { url: 'https://news.cctv.com/2019/07/gaiban/cmsdatainterface/page/economy_2.jsonp', type: 'economy' },
  { url: 'https://news.cctv.com/2019/07/gaiban/cmsdatainterface/page/china_1.jsonp', type: 'china' },
  { url: 'https://news.cctv.com/2019/07/gaiban/cmsdatainterface/page/china_2.jsonp', type: 'china' },
...
const CHINESE_RSS_FEEDS = [
  { url: 'http://www.xinhuanet.com/rss/politics.xml', src: 'Xinhua', cat: 'politics' },
  { url: 'http://www.xinhuanet.com/rss/fortune.xml', src: 'Xinhua', cat: 'finance' },
  ...
  { url: 'http://www.chinadaily.com.cn/rss/business_rss.xml', src: 'ChinaDaily', cat: 'finance' },
    ...
    try {
      ...
      for (var j = 0; j < parsed.items.length; j++) {
        ...
        all.push({
          ...
          source: feed.src,
          trust: 'A',
          heat: Math.max(40, 96 - Math.floor(Math.random() * 30)),
          minsAgo: Math.max(1, minsAgo),
          url: item.link || '',
          cluster: detectCluster(title),
          debunk: false,
...
const rssParser = new RssParser({ timeout: 15000, headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36', 'Accept': 'application/rss+xml,application/xml,text/xml,*/*' } });

const RSS_FEEDS = [
  { url: 'http://feeds.bbci.co.uk/news/world/rss.xml', src: 'BBC', cat: 'politics' },
  { url: 'http://feeds.bbci.co.uk/news/business/rss.xml', src: 'BBC', cat: 'finance' },
  { url: 'http://rss.cnn.com/rss/edition.rss', src: 'CNN', cat: 'politics' },
  { url: 'http://rss.cnn.com/rss/money_latest.rss', src: 'CNN', cat: 'finance' },
  { url: 'https://www.theguardian.com/world/rss', src: 'Guardian', cat: 'politics' },
  { url: 'https://www.theguardian.com/uk/business/rss', src: 'Guardian', cat: 'finance' },
  { url: 'https://www.aljazeera.com/xml/rss/all.xml', src: 'AlJazeera', cat: 'politics' },
  { url: 'https://www.france24.com/en/rss', src: 'AP', cat: 'politics' },
  { url: 'https://rss.dw.com/rdf/rss-en-all', src: 'AP', cat: 'politics' },
  { url: 'http://feeds.reuters.com/reuters/topNews', src: 'Reuters', cat: 'politics' },
  { url: 'http://feeds.reuters.com/reuters/businessNews', src: 'Reuters', cat: 'finance' },
  ...
  for (var i = 0; i < RSS_FEEDS.length; i++) {
    ...
    try {
      ...
      for (var j = 0; j < parsed.items.length; j++) {
        ...
        if (minsAgo > MAX_AGE_HOURS * 60) continue;
        all.push({
          title: title, source: feed.src, trust: 'A',
          heat: Math.max(30, 95 - Math.floor(Math.random() * 40)),
          minsAgo: Math.max(1, minsAgo), url: item.link || '',
          cluster: detectCluster(title), debunk: false,
          sourceFull: getSrc(feed.src).full,
          sourceColor: getSrc(feed.src).color,
          sourceLabel: getSrc(feed.src).label,
          category: feed.cat, hotScore: 0
        });
      }
    } catch (e) { /* skip broken feeds */ }
  }
...
async function fetchTrending(domain, category) {
  ...
  if (domain === 'domestic') {
    ...
    var cnRssArticles = await fetchChineseRSS();
    var cnRssFiltered = filterByCategory(cnRssArticles, category);
    articles = cctvArticles.concat(cnRssFiltered);
  } else {
  ...
  var totalArticles = articles.length; articles.forEach(function(a) { var crossRef = articles.filter(function(x) { return x !== a && detectCluster(a.title) === detectCluster(x.title) && detectCluster(a.title); }).length; a.hotScore = calcHotScore(a.heat, a.minsAgo, a.trust); a.heatComponents = { timeFresh: Math.round(Math.max(0, 24 - (a.minsAgo||0)/60) / 24 * 100), propagation: Math.round(Math.min(1, totalArticles/500) * 100), authority: Math.round(Math.min(1, crossRef/10) * 100), srcWeight: Math.round(({A:100,B:80,C:40}[a.trust]||80) / 100 * 100) }; });
