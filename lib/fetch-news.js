...
async function fetchNews(domain, category) {
  ...
  try {
    if (domain === 'domestic') {
      ...
      const cat = category === 'military' ? null : category;
      let path;
      if (category === 'military') {
        path = `/v2/everything?q=military+defense&sortBy=publishedAt&pageSize=20&apiKey=${NEWSAPI_KEY}`;
      } else {
        const newsCat = cat === 'finance' ? 'business' : 'general';
        path = `/v2/top-headlines?country=us&category=${newsCat}&pageSize=20&apiKey=${NEWSAPI_KEY}`;
      }
