...
async function fetchTrending(domain, category) {
  ...
  var total = articles.length; articles.forEach(function(a) { a.hotScore = calcHotScore(a.heat, a.minsAgo, a.trust); a.heatComponents = { timeFresh: Math.round(Math.max(0, 24 - (a.minsAgo||0)/60) / 24 * 100), propagation: Math.round(Math.min(1, total/500) * 100), authority: Math.round(Math.min(1, (a.trust==="A"?10:5)/10) * 100), srcWeight: Math.round(({A:100,B:80,C:40}[a.trust]||50)) }; });
